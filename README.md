...
2. Edit configuration files. The following configuration files need to be modified in _command_node_tools/config_files_:
   ...
   - _capture_config.py_: Interface name for packet capture on each cluster node. Edit ssh information here too (used by cluster nodes to log into command node).
...
# Running the code
1. The following command reads the relevant configuration files and runs the main flow for each unit test:
   ```
