...
for node in nodes:
    ...
    subprocess.run(['scp', '-i', id_rsa_location,'-o','StrictHostKeyChecking=no','node0:~/servers_clients/*', destination ])
...
for node in nodes:
    ...
    subprocess.run(['sudo','scp','-o','StrictHostKeyChecking=no','node0:~/network_nodes/chrony.conf', chrony_conf_destination ])
